######################################################################################
#    CPEM： Category-Preserved Entropy Minimization for Unsupervised Domain Adaptation
...
#    All rights reserved, please report any debug problem to (will be available soon)
######################################################################################
...
def train(epoch, model, ws, pt):
    ...
    pb_cls_tgt = torch.FloatTensor(pt)
    pb_cls_tgt = pb_cls_tgt.cuda()
    ...
    while i <= len_source_loader:
        ...
        ws_batch = Variable(torch.FloatTensor(ws_batch).cuda())
        ...
        ## Target category-divergence loss
        pb_pred_tgt = clabel_tgt.sum(dim=0)
        pb_pred_tgt = 1.0/pb_pred_tgt.sum() * pb_pred_tgt  #normalizatoin to a prob. dist.
        target_div_loss = 0.5 * (torch.sum(pb_pred_tgt * torch.log(pb_pred_tgt/pb_cls_tgt + 1e-8))\
                         + torch.sum(pb_cls_tgt * torch.log(pb_cls_tgt/(pb_pred_tgt + 1e-8))))

        target_entropy_loss= -torch.mean((clabel_tgt * torch.log(clabel_tgt + 1e-6)).sum(dim=1))
        total_loss = label_loss + 1.0 * target_entropy_loss + 0.5 * target_div_loss  #(0.5,1): 70.25%---max
